...
namespace Kritik
{
    public static class DataLoadSaveOld
    {
        /// <summary>
        ...
        public static bool UlozitVysledky(string fileName, Hridel hridel)
        {
            ...
            {
                { Hridel.opVolnyKeyword, Texts.VOLNY },
                { Hridel.opKloubKeyword, Texts.KLOUB },
                { Hridel.opVetknutiKeyword, Texts.VETKNUTI }
            };
            Dictionary<string, string> gyrosVypsatDict = new()
            {
                { Hridel.gyrosZanedbaniKeyword, Texts.VLIVGYROSNENIUVAZOVAN },
                { Hridel.gyrosSoubeznaKeyword, Texts.SOUBEZNAPRECESE },
                { Hridel.gyrosProtibeznaKeyword, Texts.PROTIBEZNAPRECESE }
            };
            ...
            try
            {
                using (ExcelPackage p = new ExcelPackage(fileInfo))
                {
                    ...
                    if (hridel.KritOt.Length == 0) { ws.Cells[++row, 2].Value = Texts.NebylyVypoctenyZadneKritickeOtacky; }
                        ...
                        foreach (Hridel.PrvekTab a in hridel.PrvkyHrideleTab)
                        {
                            ...
                            switch (a.Typ)
                            {
                                ...
                                case Hridel.beamPlusKeyword:
                                    ...
                                    ws.Cells[row, 8].Style.HorizontalAlignment = alignLeft;
                                    ws.Cells[row, 8].Style.Numberformat.Format = formatNum;
                                    ws.Cells[++row, 2].Value = Texts.HridelJeRozdelenaNa + " " + (a.Deleni + 1) + " " + Texts.castiODelce + " " + string.Format("{0:#.###}", a.L / (a.Deleni + 1)) + " mm, " + Texts.meziKterymiJsouUmistenyPrvkyDT;
                                    if (a.M > 0)
                                    {
